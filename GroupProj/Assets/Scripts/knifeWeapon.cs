...
public class knifeWeapon : MonoBehaviour
{
    ...
    private void OnEnable()
    {
        ...
        animator.SetTrigger("takeOutKnife");
        //isReloading = false;
        animator.ResetTrigger("knifeAttack");
        animator.SetBool("knifePrep", false);
    ...
    void Update()
    {
        if (kPickup.hasPickedUpK)
        {
            ...
            //    return;
            //}
            if (Input.GetButtonDown("Shoot") && !isShooting && !isAiming)//Time.time >= nextTimeToFire)
            {
    ...
    {
        animator.SetBool("knifePrep", true);


        if (Input.GetButtonDown("Shoot") && isAiming)
        {
            //Throw Knife
